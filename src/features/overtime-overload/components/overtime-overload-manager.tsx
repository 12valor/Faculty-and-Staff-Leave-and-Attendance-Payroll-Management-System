...
import { decideOverloadAction, decideOvertimeAction, generateOverloadAction, generateOvertimeAction } from "@/features/overtime-overload/actions";
...
function Summary({ title, value }: { title: string; value: string }) { return <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{title}</CardTitle></CardHeader><CardContent><p className="font-mono text-2xl font-semibold">{value}</p></CardContent></Card>; }
function RecordTable({ headers, empty, children }: { headers: string[]; empty: string; children: React.ReactNode }) { const hasRows = Array.isArray(children) ? children.length > 0 : Boolean(children); return <div className="overflow-x-auto rounded-xl border bg-card"><Table><TableHeader><TableRow>{headers.map((header) => <TableHead key={header}>{header}</TableHead>)}</TableRow></TableHeader><TableBody>{hasRows ? children : <TableRow><TableCell colSpan={headers.length} className="h-24 text-center text-muted-foreground">{empty}</TableCell></TableRow>}</TableBody></Table></div>; }
function EmployeeCell({ name, department }: { name: string; department: string }) { return <TableCell><p className="font-medium">{name}</p><p className="text-xs text-muted-foreground">{department}</p></TableCell>; }
function Status({ value }: { value: string }) { return <TableCell><Badge variant={value === "APPROVED" ? "secondary" : "outline"}>{value}</Badge></TableCell>; }
function DecisionButtons({ pending, onApprove, onReject }: { pending: boolean; onApprove: () => void; onReject: () => void }) { return <TableCell>{pending ? <div className="flex gap-1"><Button size="icon-sm" variant="ghost" aria-label="Approve" onClick={onApprove}><Check /></Button><Button size="icon-sm" variant="ghost" aria-label="Reject" onClick={onReject}><X /></Button></div> : "—"}</TableCell>; }
