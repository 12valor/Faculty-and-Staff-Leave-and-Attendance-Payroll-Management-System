...
export function ManualAttendanceForm({ employees, initialValues }: { employees: Array<{ id: string; label: string }>; initialValues: AttendanceEntryValues }) {
  ...
  async function calculate() {
    try {
      const result = await previewAttendanceAction(form.getValues());
      if (!result.ok || !result.preview) return toast.error(result.error ?? "Unable to preview attendance.");
      setPreview(result.preview);
    } catch {
      toast.error(SERVER_ACTION_ERROR);
    }
  }
  async function submit(values: AttendanceEntryValues) {
    try {
      const result = await saveManualAttendanceAction(values);
      if (!result.ok) return toast.error(result.error);
      toast.success(values.id ? "Attendance updated." : "Attendance saved.");
