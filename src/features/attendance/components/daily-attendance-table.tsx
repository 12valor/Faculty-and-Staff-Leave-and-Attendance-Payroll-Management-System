...
export function StatusBadge({ status, overridden = false, leaveType, isFutureDate = false }: { status: AttendanceStatus; overridden?: boolean; leaveType?: string | null; isFutureDate?: boolean }) {
  if (isFutureDate && status === "ABSENT") {
    return <Badge variant="secondary">Upcoming{overridden ? " · Override" : ""}</Badge>;
  }
