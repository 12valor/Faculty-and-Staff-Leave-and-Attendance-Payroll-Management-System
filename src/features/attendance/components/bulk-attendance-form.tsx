...
export function BulkAttendanceForm({ employees }: { employees: Array<{ id: string; label: string }> }) {
  ...
  async function submit(values: BulkAttendanceValues) {
    const rows = values.rows.filter((_, index) => selected.has(index));
    if (!rows.length) return toast.error("Select at least one employee.");
    try {
      const result = await saveBulkAttendanceAction(rows);
      if (!result.ok) return toast.error(result.error);
      toast.success(`${result.count} attendance records saved.`);
