...
import { createPayrollPeriodAction, generatePayrollAction, lockPayrollPeriodAction, previewPayrollAction, type PayrollPreviewRow } from "@/features/payroll/actions";
...
export function PayrollManager({ periods }: { periods: PeriodRow[] }) {
  ...
  <Dialog open={previewOpen} onOpenChange={setPreviewOpen}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-5xl"><DialogHeader><DialogTitle>{previewName} deductions</DialogTitle><DialogDescription>Employee summaries with attendance and unpaid-leave breakdowns.</DialogDescription></DialogHeader><div className="flex flex-col gap-4">{previewRows.length ? previewRows.map((row) => <div key={row.employeeId} className="rounded-xl border"><div className="grid gap-3 p-4 md:grid-cols-4"><div><p className="font-medium">{row.employee}</p><p className="text-xs text-muted-foreground">Salary ₱{row.monthlySalary.toFixed(2)}</p></div><Metric label="Daily rate" value={`₱${row.dailyRate.toFixed(2)}`} /><Metric label="Penalty / LWOP units" value={row.dayValue.toFixed(3)} /><Metric label="Deduction" value={`₱${row.amount.toFixed(2)}`} /></div><div className="overflow-x-auto border-t"><Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Source</TableHead><TableHead>Late</TableHead><TableHead>Undertime</TableHead><TableHead>Absence</TableHead><TableHead>LWOP</TableHead><TableHead>Penalty / LWOP units</TableHead><TableHead>Amount</TableHead></TableRow></TableHeader><TableBody>{row.breakdowns.map((item) => <TableRow key={item.date}><TableCell className="font-mono">{item.date}</TableCell><TableCell>{item.source.replaceAll("_", " ")}</TableCell><TableCell>{item.lateMinutes ?? 0}</TableCell><TableCell>{item.undertimeMinutes ?? 0}</TableCell><TableCell>{item.absenceDayValue ?? 0}</TableCell><TableCell>{item.lwopDayValue ?? 0}</TableCell><TableCell>{item.dayValue.toFixed(3)}</TableCell><TableCell>₱{item.amount.toFixed(2)}</TableCell></TableRow>)}</TableBody></Table></div></div>) : <p className="rounded-lg bg-muted p-6 text-center text-muted-foreground">No deductions for this period.</p>}</div></DialogContent></Dialog></>;
}
function Metric({ label, value }: { label: string; value: string }) { return <div><p className="text-xs text-muted-foreground">{label}</p><p className="font-mono font-medium">{value}</p></div>; }
