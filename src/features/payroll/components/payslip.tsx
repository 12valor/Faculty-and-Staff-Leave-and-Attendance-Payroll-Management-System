    ...
    <article data-payslip className="mx-auto w-full max-w-5xl rounded-xl border bg-white p-5 text-slate-950 shadow-sm sm:p-8">
      ...
      <section className="mt-6 grid gap-5 lg:grid-cols-2">
        ...
        <SummaryTable title="Deductions" rows={[
          [`Late (${payroll.deductions.totalLateMinutes} min) and undertime (${payroll.deductions.totalUndertimeMinutes} min)`, deductionPart(payroll, "time")],
...
function deductionPart(payroll: LivePayrollResult, kind: "time" | "absence" | "lwop") { return payroll.deductionRows.filter((row) => kind === "time" ? row.lateMinutes > 0 || row.undertimeMinutes > 0 : kind === "absence" ? row.absenceDayValue > 0 : row.lwopDayValue > 0).reduce((sum, row) => sum + row.amount, 0); }
