...
export function Payslip({ payroll, preparedBy }: { payroll: LivePayrollResult; preparedBy: string }) {
  ...
  return (
    <article data-payslip className="mx-auto w-full max-w-5xl rounded-xl border bg-white p-5 text-slate-950 shadow-sm sm:p-8 print:border-none print:shadow-none">
      ...
      <section className="mt-6 grid gap-x-8 gap-y-3 rounded-lg bg-slate-50 p-4 text-sm sm:grid-cols-2 print:mt-3 print:p-2">
      ...
      <section className="mt-6 grid gap-5 lg:grid-cols-2 print:mt-3 print:gap-3">
      ...
      <div className="mt-5 flex flex-col justify-between gap-4 rounded-lg bg-primary px-5 py-4 text-primary-foreground sm:flex-row sm:items-center print:mt-3 print:px-3 print:py-2"><div><p className="text-xs font-semibold tracking-wider uppercase opacity-80">Net pay</p><p className="mt-1 text-sm opacity-80">Gross earnings less attendance and LWOP deductions</p></div><p className="font-mono text-3xl font-bold tabular-nums">{money.format(payroll.netPay)}</p></div>

      <section className="mt-6 print:mt-3"><h3 className="mb-3 text-sm font-bold tracking-wider uppercase text-slate-600 print:mb-1">Payroll basis</h3><div className="grid gap-3 rounded-lg border p-4 text-sm sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 print:p-2"><Detail label="Monthly salary" value={money.format(payroll.employee.monthlySalary)} mono /><Detail label="Daily rate" value={money.format(payroll.basis.dailyRate)} mono /><Detail label="Days present" value={`${payroll.basis.daysPresent}`} mono /><Detail label="SL used / bal." value={`${payroll.basis.sickUsed} / ${payroll.basis.sickBalance}`} mono /><Detail label="VL used / bal." value={`${payroll.basis.vacationUsed} / ${payroll.basis.vacationBalance}`} mono /><Detail label="Total paid leave" value={`${payroll.basis.paidLeaveDays}`} mono /><Detail label="Total paid days" value={`${payroll.basis.totalPaidDays}`} mono /></div></section>
      ...
      {payroll.overtimeRows.length ? <DetailTable title="Overtime earnings" headers={["Date", "Hours", "Amount"]}>{payroll.overtimeRows.map((row) => <TableRow key={row.date}><TableCell className="font-mono text-xs">{row.date}</TableCell><TableCell>{row.hours.toFixed(3)}</TableCell><TableCell className="font-mono">{money.format(row.amount)}</TableCell></TableRow>)}</DetailTable> : null}
      ...
      <footer className="mt-12 grid gap-12 pt-4 text-center text-sm sm:grid-cols-2 print:mt-6 print:gap-6 print:pt-2"><Signature label="Prepared by" name={preparedBy} /><Signature label="Employee signature" name={payroll.employee.fullName} /></footer>
      <p className="mt-8 text-center text-[0.65rem] text-slate-500 print:mt-4">{payroll.mode === "manual" ? "Manual" : "Live"} payroll generated {generatedAt}. This view does not create or lock a payroll record.</p>
    </article>
  );
}
...
function SummaryTable({ title, rows, totalLabel, total }: { title: string; rows: Array<[string, number]>; totalLabel: string; total: number }) { return <div className="overflow-hidden rounded-lg border"><div className="border-b bg-slate-50 px-4 py-3 print:py-2"><h3 className="font-bold">{title}</h3></div><div className="p-4 print:p-2">{rows.map(([label, value]) => <div key={label} className="flex justify-between gap-4 border-b py-2 text-sm last:border-b-0 print:py-1"><span className="text-slate-600">{label}</span><span className="font-mono font-medium tabular-nums">{money.format(value)}</span></div>)}<div className="mt-2 flex justify-between gap-4 border-t-2 pt-3 font-bold print:mt-1 print:pt-2"><span>{totalLabel}</span><span className="font-mono tabular-nums">{money.format(total)}</span></div></div></div>; }
function DetailTable({ title, headers, children }: { title: string; headers: string[]; children: React.ReactNode }) { return <section className="mt-7 break-inside-avoid print:mt-4"><h3 className="mb-3 text-sm font-bold tracking-wider uppercase text-slate-600 print:mb-1">{title}</h3><div className="overflow-x-auto rounded-lg border"><Table><TableHeader><TableRow>{headers.map((header) => <TableHead key={header}>{header}</TableHead>)}</TableRow></TableHeader><TableBody>{children}</TableBody></Table></div></section>; }
